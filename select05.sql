...
--식별자 / 기본키(Primary Key) PK -> 유일한 식별자
select 
    COUNT(empno),
    COUNT(mgr),
    COUNT(comm),
...
그룹화 결과에 대해 조건 -> HAVING --GROUP BY 다음에 위치
...
group by grade
order by grade desc;        --order by grade 사용해서 순서대로 정렬
