...
select empno AS "사원번호",  --empno employeeNumber
        ename "사원이름", 
        job 직업,
        sal "급여 데이터"  -- 띄어쓰기 하고 싶으면 "" 사용하기
from emp;

/********************************************/
--조회 데이터 리터럴 값 활용
select '문자', 12313 FROM 테이블명; --작은 따옴표로 문자 표현
...
select '부서번호:' || (deptno+100)  AS "부서번호" from dept; -- 객체 찾을때 SHIFT + F4
...
--where ename LIKE 'M%'
--where ename LIKE '%M';
--where ename LIKE '_M%'; -- SMITH _앞에 한글자 있는 거
where ename LIKE '__M%'; -- JAMES _앞에 두글자 있는거
...
from student
where grade IN (1,2,3)
order by grade desc, height;   --오름차순/내림차순은 각각 넣어줄 수 있음

select *
from student
where grade IN (1,2,3)
order by height, grade desc;    --height를 먼저 학년을 나중에

select *
from student
order by birthday desc; -- asc(오름차순) = 나이 많은 사람, desc(내림차순) = 젊은 사람부터 정렬
...
--101학과에 속한 학생들과 교수들을 조회  *데이터 유형이 맞아야함 
...
select studno 식별번호, name, deptno1 학과, tel   --컬럼별칭 설정은 앞쪽
from student
where deptno1 = 101
UNION ALL
select  profno, name, deptno, null  -- 데이터 값(tel) 맞춰주기 위해 null 사용
from professor
where deptno = 101
